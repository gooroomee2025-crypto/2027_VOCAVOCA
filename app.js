...
(function () {
  ...
  /* ---------------------------------------------------------
     ...
     세법OX 앱과 같은 Firebase 프로젝트를 쓰되, 컬렉션을 분리해서 데이터가 섞이지 않게 함
  --------------------------------------------------------- */
  var firebaseConfig = {
  ...
  var db = null;
  var syncCode = localStorage.getItem('vocaSyncCode') || null;
  ...
  function pullFromCloud() {
    if (!db || !syncCode) return Promise.resolve(null);
    return db.collection('vocaSyncData').doc(syncCode).get().then(function (snap) {
      return snap.exists ? snap.data() : null;
    }).catch(function () { return null; });
  }

  function pushToCloud() {
    if (!db || !syncCode) return Promise.resolve();
    return db.collection('vocaSyncData').doc(syncCode).set({
      state: state,
      updatedAt: Date.now()
  ...
  function syncOnLoad() {
    if (!db || !syncCode) return Promise.resolve();
    return pullFromCloud().then(function (cloudData) {
      var localUpdated = Number(localStorage.getItem('vocaLastUpdated') || 0);
      if (cloudData && cloudData.updatedAt > localUpdated) {
        state = Object.assign(defaultState(), cloudData.state);
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (e) { /* ignore */ }
        localStorage.setItem('vocaLastUpdated', String(cloudData.updatedAt));
      } else {
        return pushToCloud().then(function () {
          localStorage.setItem('vocaLastUpdated', String(Date.now()));
        });
      }
    });
  }
  ...
  function setupSync() {
    var input = window.prompt(
      '다른 기기와 동기화할 코드를 입력하세요.\n(같은 코드를 입력한 기기끼리만 서로 연동됩니다. 형제자매 등 다른 사람과 같이 쓸 경우 서로 다른 코드를 정해주세요.)',
      syncCode || ''
    );
    ...
    syncCode = trimmed;
    localStorage.setItem('vocaSyncCode', syncCode);
  ...
  function renderSettings() {
    ...
    wrap.appendChild(h('div', { class: 'settings-group' }, [
      text('div', 'settings-group-title', '기기 간 동기화'),
      h('div', { class: 'settings-row' }, [
        h('div', { class: 'settings-row-text' }, [
          text('div', 'settings-row-title', '🔗 동기화 코드'),
          text('div', 'settings-row-desc', syncCode ? ('연동 코드: ' + syncCode) : '아직 설정되지 않음 — 다른 기기와 진행 상황을 공유하려면 설정하세요.')
        ]),
        h('button', {
          class: 'settings-btn neutral', type: 'button',
          onclick: setupSync
        }, [syncCode ? '변경' : '설정'])
      ])
    ]));
